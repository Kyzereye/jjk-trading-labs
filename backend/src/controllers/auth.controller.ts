...
router.put('/preferences', authMiddleware, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  await dbService.execute(
    `UPDATE user_preferences SET 
     ...
     enable_adx_filter = ?,
     adx_min_value = ?,
     enable_consecutive_close_filter = ?,
     required_consecutive_closes = ?
     WHERE user_id = ?`,
    [
      ...
      preferences.discovery_min_win_rate ?? 50.0,
      preferences.discovery_min_return ?? 5.0,
      preferences.discovery_min_sharpe ?? 0.20,
      preferences.discovery_min_trades ?? 3,
      preferences.discovery_max_stocks ?? 15,
      preferences.enable_atr_filter ?? false,
      preferences.atr_min_percentile ?? 50.0,
      preferences.enable_adx_filter ?? false,
      preferences.adx_min_value ?? 25.0,
      preferences.enable_consecutive_close_filter ?? false,
      preferences.required_consecutive_closes ?? 2,
      userId
    ]
  );
